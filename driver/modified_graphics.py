# this is modified a bit from the base library. original can be found here
# http://www.science.smith.edu/dftwiki/index.php/Zelle%27s_Graphics.py_for_Python_3
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        self.bind("<ButtonRelease-1>", self._onLeftClickRelease)
        self.bind("<Button-3>", self._onRightClick) # right click
        self.bind("<ButtonRelease-3>", self._onRightClickRelease)
    ...
    def _onKeyDown(self, evnt):
        ...
        if evnt.keysym not in self.all_keys_down:
            self.all_keys_down.add(evnt.keysym)

    def _onKeyRelease(self, evnt):
        self.lastKey = ""
        self.all_keys_down.discard(evnt.keysym)
    ...
    def checkLeftRelease(self):
        ...
        self.update()
        if self.leftreleaseX != None and self.leftreleaseY != None:
            x,y = self.toWorld(self.leftreleaseX, self.leftreleaseY)
            self.leftreleaseX = None
            self.leftreleaseY = None
            return Point(x,y)
        else:
            return None
    ...
    def checkKeyList(self):
        """Return last key pressed or None if no key pressed since last call"""
        if self.isClosed():
            raise GraphicsError("checkKey in closed window")
        self.update()
        return list(self.all_keys_down)
    ...
    # TODO: this is a waste of processing but i don't want to fix it right now. should share mouseX and mouseY
    ...
    def _onLeftClickRelease(self, e):
        self.left_click_down = False
        self.leftreleaseX = e.x
        self.leftreleaseY = e.y
