...
## current issue log 
    All issues classified as either: a. Bug, or b. Failure (to chess requirements), or c. Sloth (slowdown) or d. Feature. ~~Crossed out~~ items are done, but under testing.

  * Sloth: Move structure is to big (56 bytes) . For example look at 
  ...
  * Bug: Asserts do not work in gameboard.cpp. No idea why, Eben when I #UNDEF NDEBUG every where it does not help.
  * Bug: Disapperaing pieces. Only in the python, display, peices are really there. Infrequent. But reepeatiable, 
...
## recommended usage (for python scripts/run_tests.py -dD and -tT)
  fast 5 minute play: -d6 -t10      (or autoplay of computers)
